...
class XieCheng():
    ...
    def __init__(self, roomid,cityid=2):
        ...
        self.address = ''.join([item.strip() for item in  etree.HTML(self.session.get(url).text).xpath('//div[@class="adress"]//text()') if item.strip()])
    ...
    def get_eleven_js(self):
        url = 'http://hotels.ctrip.com/domestic/cas/oceanball?callback={}&_={}'.format(self.call_back_value,self.get_ms())
    ...
    def get_room(self):
        ...
        params = {'MasterHotelID': self.roomid, 'hotel': self.roomid,
                  'city': self.cityid,
                  'startDate': str(self.start_date),
                  'depDate': str(self.dep_date),
                  'RoomGuestCount': '1,1,0',
                  'eleven': eleven,
                  'callback': self.call_back_value, '_': '{}'.format(self.get_ms())}
